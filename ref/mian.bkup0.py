        ...
        if(flag_run):
            ...
            if("(VES) VERTICAL ELECTRICAL SOUNDING" in dt_mode):
                if(flag_measure == False):
                    ...
                    # Clock.schedule_interval(self.check_serial_data, TIMEOUT_MCU)
                flag_measure = True
    ...
    def inject_current_event(self, dt):
        ...
        if inject_state in {0, 4, 8, 12, 16, 20, 24, 28, 32, 36}:
            ...
            if(not DEBUG):
                ...
                while serial_retry > 0:
                    # data_stop_inject = data_serial_read
                    # Logger.info(data_stop_inject)
                    serial_retry -= 1
            ...
            if(not DEBUG):
                ...
                serial_retry = RETRY_READ_MCU
                while serial_retry > 0:
                    # data_plus_inject = data_serial_read
                    # Logger.info(data_plus_inject)   
                    serial_retry -= 1
                ...
                serial_retry = RETRY_READ_MCU
                while serial_retry > 0:
                    # data_indikasi_lanjut = data_serial_read
                    # Logger.info(data_indikasi_lanjut)
                    serial_retry -= 1   
            ...
            if(not DEBUG):
                ...
                serial_retry = RETRY_READ_MCU
                while serial_retry > 0:
                    # data_negatif_inject = data_serial_read
                    # Logger.info(data_negatif_inject) 
                    serial_retry -= 1
