...
import minimalmodbus, configparser, serial, logging, asyncio
...
class ScreenData(MDScreen):
    ...
    def ensure_asyncio_event_loop(self):
        try:
    ...
    def schedule_async_task(self, async_func, *args, **kwargs):
        ...
        if(flag_run):
            ...
            if("(VES) VERTICAL ELECTRICAL SOUNDING" in dt_mode):
                if(flag_measure == False):
                    ...
                    Clock.schedule_once(lambda dt: self.schedule_async_task(self.inject_current_event, dt))
                flag_measure = True
    ...
    async def inject_current_event(self, dt):
        global inject_state, step
        global dt_cycle, dt_time
        global com_port_mcu

        time_sampling = (int(dt_time) / 10000)
        Logger.debug(f"ENter Inject Curr Event: {time_sampling}")
        ...
        try:
            if inject_state in {0, 4, 8, 12, 16, 20, 24, 28, 32, 36}:
                ...
                toast(toast_msg)
                if not DEBUG:
                    await self.write_and_wait_for_response(b".", "Not Injected")
                    self.switching_commands()

            elif inject_state in {1, 5, 9, 13, 17, 21, 25, 29, 33, 37}:
                if not DEBUG:
                    await self.write_and_wait_for_response(b"+", "Inject Positif")
                    await self.write_and_wait_for_response(None, "Silahkan", trigger_sampling=True)
                ...
                if not DEBUG:
                    await self.write_and_wait_for_response(b"-", "Inject Negatif")
                    await self.write_and_wait_for_response(None, "Silahkan", trigger_sampling=True)

        except Exception as e:
            Logger.error(f"Error in inject_current_event: {e}")

        inject_state += 1


    async def write_and_wait_for_response(self, command, expected_response, trigger_sampling=False):
        """
        Writes a command to the serial port and waits for the expected response asynchronously.
        """
        global com_port_mcu

        Logger.info(f"Enter Writr and Wait FUnction")

        if command:
            com_port_mcu.write(command)

        while True:
            try:
                response = com_port_mcu.readline().decode("utf-8").strip()
                Logger.info(f"Serial Response: {response}")
                ...
                Logger.error(f"Error reading serial response: {e}")
        ...
        if(flag_run):
            if (not DEBUG):
                try:
                    ...
                    data_current = str(com_port_mcu.readline().decode("utf-8").strip())  # read the incoming data and remove newline character
                ...
                try:
                    ...
                    data_millivoltage = str(com_port_mcu.readline().decode("utf-8").strip())  # read the incoming data and remove newline character
                    ...
                    while True:
                        if data_millivoltage[0] == "v":
                            ...
                            com_port_mcu.write(b"v")
                            data_millivoltage = str(com_port_mcu.readline().decode("utf-8").strip())
                except Exception as e:
