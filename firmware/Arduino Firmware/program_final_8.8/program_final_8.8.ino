...
int flag_tidak_ada_multimeter_a = 0;
int flag_tidak_ada_multimeter_v = 0;
int counter_cek1, counter_cek2 = 0;
int flag_tidak_ada_data_a = 0;
int flag_tidak_ada_data_v = 0;
int indikator_good = 0;
int awalan = 0;
...
void loop() {
  Komunikasi();
  //int flag_tidak_ada_multimerter_a = 0;
  //int flag_tidak_ada_multimerter_v = 0;
  while (flag_tidak_ada_multimeter_a == 0)
  {
    //Serial.println("cek multimeter");
    if (Serial1.available() == 0) {
      counter_cek1++;
      if (counter_cek1 >= 100) {
        flag_tidak_ada_multimeter_a = 1;
        counter_cek1 = 0;
        break;
      }
    }
  ...
  while (flag_tidak_ada_multimeter_v == 0)
  {

    if (Serial2.available() == 0) {
      counter_cek2++;
      if (counter_cek2 >= 100) {
        flag_tidak_ada_multimeter_v = 1;
        counter_cek2 = 0;
        break;
      }
    }
  ...
  if (flag_tidak_ada_multimeter_a == 1 || flag_tidak_ada_multimeter_v == 1)
  {
    if (awalan >= 2)
    {
      ...
      ampere = 0;
      volt = 0;
      flag_tidak_ada_data_a = 1;
      flag_tidak_ada_data_v = 1;
      flag_tidak_ada_multimeter_a = 0;
      flag_tidak_ada_multimeter_v = 0;
      //Serial.println("tidak ada multimeter");
      if (o <= 1000)
      {
        delay(1);
        o++;
...
int Komunikasi() {
  ...
  if (Data_komunikasi5[0] == '>') {
    if (treshold_voltage < treshold_maximum_voltage)
    {
      ...
      Serial.print("t"); Serial.println(treshold_voltage);
    }
    else
    {
      Serial.print("t"); Serial.println(treshold_voltage);
  ...
  if (Data_komunikasi5[0] == '<') {
    if (treshold_voltage > treshold_minimum_voltage)
    {
      treshold_voltage = treshold_voltage - 5.00;
      Serial.print("t"); Serial.println(treshold_voltage);
    }
