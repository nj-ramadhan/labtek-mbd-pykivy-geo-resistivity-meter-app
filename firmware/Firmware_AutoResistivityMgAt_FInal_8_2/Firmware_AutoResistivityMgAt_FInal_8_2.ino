...
void loop() {
  ...
  /*
    ...
    else if(AVGTegangan <= Threshold && LevelTegangan == 2){
    digitalWrite(Relay110, HIGH);
    digitalWrite(Relay172, LOW);
    digitalWrite(Relay200, HIGH);
    digitalWrite(Relay220, HIGH);
    ...
    else if(AVGTegangan <= Threshold && LevelTegangan == 3){
    digitalWrite(Relay110, HIGH);
    digitalWrite(Relay172, HIGH);
    digitalWrite(Relay200, LOW);
    digitalWrite(Relay220, HIGH);
    ...
    else if(AVGTegangan <= Threshold && LevelTegangan == 4){
    digitalWrite(Relay110, HIGH);
    digitalWrite(Relay172, HIGH);
    digitalWrite(Relay200, HIGH);
    digitalWrite(Relay220, LOW);
    ...
    else if(AVGTegangan <= Threshold && LevelTegangan == 5){
    digitalWrite(Relay110, HIGH);
    digitalWrite(Relay172, HIGH);
    digitalWrite(Relay200, HIGH);
    digitalWrite(Relay220, HIGH);
    ...
    TotalTegangan = 0;
  */
  /*for(int i = 1; i <= BanyakSampling; i++ ){
    // ADCinput =analogRead(Vread);
    // Tegangan = map(ADCinput, 0, 1023, 0, 5000);
    Tegangan = volt;
    TotalTegangan = TotalTegangan + Tegangan;
    delay(IntervalSampling);
    // Serial.println(Tegangan);
    }

    Serial.println(" ");
    AVGTegangan = TotalTegangan/BanyakSampling;
    Serial.println(AVGTegangan);
  */
}
...
int Komunikasi() {
  ...
  if (perintah_minta_data2[0] == '+') {
    ...
    if (treshold_voltage < treshold_maximum_voltage) {
      ...
      Serial.print("(+5) set threshold inject = ");
      Serial.println(treshold_voltage);
    } else {
      Serial.print("Threshold inject Maximum = ");
      Serial.println(treshold_voltage);
    }
  }
  if (perintah_minta_data2[0] == '-' && treshold_voltage >= 10.00) {
    ...
    if (treshold_voltage > treshold_maximum_voltage) {
      ...
      Serial.print("Threshold inject Minimum = ");
      Serial.println(treshold_voltage);
    }
  }
  if (perintah_minta_data2[0] == '_') {
    Serial.print("threshold inject = ");
    Serial.println(treshold_voltage);
  }
  if (Data_komunikasi2[0] == 'v' && indicator_voltase_cukup == 1) {
    ...
    Serial.println(ampere_kirim2);
    Serial.print(" | ");
    Serial.println(detik_pembacaaan);
  }
